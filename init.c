...
int pintos_init(void)
{
  ...
  else
  {
    ...
    printf("Welcome to the Pintos OS...\n");
    while (1)
    {
      printf("CS2042> ");
      uint8_t input[100];
      uint8_t *StringsToMatch[] = {"whoami", "shutdown", "time", "ram", "thread", "priority", "exit"} ;
      int i = 0;
      while (1)
      {
      ...
      input[i] = '\0';
        ...
        if (strcmp((char *)input, (char *)StringsToMatch[0]) == 0) //Check for "whoami"
        {
          printf("JAYATHILAKE.N.C.- 210257F\n");
        }
        ...
        else if (strcmp((char *)input, (char *)StringsToMatch[5]) == 0) //Check for "priority"
        {
          int priority= thread_get_priority ();
          printf("Priority of the current thread is: %d\n",priority);
        }

        else if (strcmp((char *)input, (char *)StringsToMatch[6]) == 0) //Check for "exit"
        {
          printf("Exiting Interactive Shell...Bye!\n");
          break;
        }
        
        else // Check for any other command
        {
